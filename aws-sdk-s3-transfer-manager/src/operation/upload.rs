...
mod test {
    ...
    #[tokio::test]
    async fn test_upload_metrics_integration() {
        use crate::io::InputStream;
        use crate::metrics::unit::ByteUnit;
        use crate::types::ConcurrencyMode;
        use crate::types::PartSize;
        ...
        assert_eq!(tm.metrics().transfers_initiated(), initial_initiated + 1);
        assert_eq!(tm.metrics().active_transfers(), initial_active + 1);
        ...
        // Check final metrics
        assert_eq!(tm.metrics().transfers_initiated(), initial_initiated + 1);
        assert_eq!(tm.metrics().transfers_completed(), initial_completed + 1);
        assert_eq!(
            tm.metrics().total_bytes_transferred(),
            initial_bytes + body.len() as u64
        );
        assert_eq!(tm.metrics().active_transfers(), initial_active); // Should be back to initial
