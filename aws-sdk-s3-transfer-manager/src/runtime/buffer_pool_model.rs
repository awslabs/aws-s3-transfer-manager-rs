...
/// Shared allocation behind private, disjoint range capabilities.
///
/// The unsafe implementation is deliberately small so the ownership contract
/// can be exercised under Miri before production arena machinery exists.
struct WindowBacking {
    data: Box<[std::cell::UnsafeCell<u8>]>,
    returns: std::sync::Arc<std::sync::atomic::AtomicUsize>,
}

// Access is safe only through ExclusiveRangeModel and FrozenWindowModel.
// Exclusive ranges are linear and can only split into non-overlapping ranges;
// freezing consumes exclusive access before exposing an immutable slice.
unsafe impl Send for WindowBacking {}
unsafe impl Sync for WindowBacking {}

impl Drop for WindowBacking {
    fn drop(&mut self) {
        self.returns
...
/// Linear mutable authority over exactly one range of a backing allocation.
///
/// This type is intentionally not Clone. Its private constructors and
/// consuming operations are the safety boundary for WindowBacking.
struct ExclusiveRangeModel {
    backing: std::sync::Arc<WindowBacking>,
    range: std::ops::Range<usize>,
}

impl ExclusiveRangeModel {
    fn whole(backing: std::sync::Arc<WindowBacking>) -> Self {
    ...
    fn as_mut(&mut self) -> &mut [u8] {
        ...
        // SAFETY: this linear capability is the sole mutable authority for
        // this range. split_at consumes it before creating disjoint children,
        // and freeze consumes it before exposing immutable access.
    ...
    fn freeze(self) -> FrozenWindowModel {
        FrozenWindowModel {
            backing: self.backing,
            range: self.range,
        }
...
struct WritableCarrierModel {
    exclusive: Option<ExclusiveRangeModel>,
    initialized: usize,
    published: bool,
}

impl WritableCarrierModel {
    fn new(capacity: usize, returns: std::sync::Arc<std::sync::atomic::AtomicUsize>) -> Self {
        assert!(capacity > 0);
        let data = (0..capacity)
            .map(|_| std::cell::UnsafeCell::new(0))
            .collect::<Vec<_>>()
            .into_boxed_slice();
        let backing = std::sync::Arc::new(WindowBacking { data, returns });
        Self {
            exclusive: Some(ExclusiveRangeModel::whole(backing)),
            initialized: 0,
            published: false,
        }
    }
    ...
    fn writable_tail(&mut self) -> &mut [u8] {
        &mut self
            .exclusive
            .as_mut()
            .expect("cursor retains its exclusive range")
            .as_mut()[self.initialized..]
    }

    fn advance_initialized(&mut self, initialized: usize) {
        let remaining = self
            .exclusive
            .as_ref()
            .expect("cursor retains its exclusive range")
            .len()
            - self.initialized;
        assert!(initialized <= remaining);
        self.initialized += initialized;
    }

    fn publish_prefix(&mut self, len: usize) -> bytes::Bytes {
        assert!(len > 0);
        ...
        let exclusive = self
            ...
            .take()
            .expect("cursor retains its exclusive range");
        let (published, remainder) = exclusive.split_at(len);
        self.exclusive = Some(remainder);
        self.initialized -= len;
        self.published = true;
        bytes::Bytes::from_owner(published.freeze())
    }

    fn seal_full_or_body_end(mut self, body_end: bool) -> bytes::Bytes {
        assert_eq!(
            self.published, false,
            "aligned emission seals one owner rather than prior short windows"
        );
        ...
        let exclusive = self
            ...
            .expect("cursor retains its exclusive range");
        assert!(
            body_end || self.initialized == exclusive.len(),
            "a non-final aligned carrier must be full"
        );
        let (initialized, unused_tail) = exclusive.split_at(self.initialized);
        drop(unused_tail);
        bytes::Bytes::from_owner(initialized.freeze())
    }
}

struct FrozenWindowModel {
    backing: std::sync::Arc<WindowBacking>,
    range: std::ops::Range<usize>,
}

impl AsRef<[u8]> for FrozenWindowModel {
    fn as_ref(&self) -> &[u8] {
        let ptr = unsafe {
            self.backing
                .data
        ...
        // SAFETY: FrozenWindowModel can only be constructed by consuming an
        // ExclusiveRangeModel, so no mutable capability overlaps this range.
...
mod tests {
    ...
    fn window_backing(capacity: usize, returns: Arc<AtomicUsize>) -> Arc<WindowBacking> {
        let data = (0..capacity)
            .map(|_| std::cell::UnsafeCell::new(0))
            .collect::<Vec<_>>()
            .into_boxed_slice();
        Arc::new(WindowBacking { data, returns })
    }

    #[test]
    fn disjoint_exclusive_ranges_can_mutate_concurrently() {
        let returns = Arc::new(AtomicUsize::new(0));
        let whole = ExclusiveRangeModel::whole(window_backing(8, Arc::clone(&returns)));
        let (left, right) = whole.split_at(4);

        let (left, right) = std::thread::scope(|scope| {
            let left = scope.spawn(move || {
                let mut left = left;
                left.as_mut().copy_from_slice(b"left");
                left
            });
            let right = scope.spawn(move || {
                let mut right = right;
                right.as_mut().copy_from_slice(b"rite");
                right
            });
            (left.join().unwrap(), right.join().unwrap())
        });

        let left = Bytes::from_owner(left.freeze());
        let right = Bytes::from_owner(right.freeze());
        assert_eq!(&left[..], b"left");
        assert_eq!(&right[..], b"rite");
        drop(left);
    ...
    #[test]
    fn rustls_style_discard_can_freeze_plaintext_and_retain_input_tail() {
        let returns = Arc::new(AtomicUsize::new(0));
        let mut input = ExclusiveRangeModel::whole(window_backing(9, Arc::clone(&returns)));
        input.as_mut().copy_from_slice(b"HabcTtail");

        // Model a consumed TLS record [header | plaintext | tag] followed by
        // encrypted input that Rustls may still mutate.
        let (consumed, mut encrypted_tail) = input.split_at(5);
        let (header, payload_and_tag) = consumed.split_at(1);
        let (plaintext, tag) = payload_and_tag.split_at(3);
        drop(header);
        drop(tag);

        let plaintext = Bytes::from_owner(plaintext.freeze());
        encrypted_tail.as_mut().copy_from_slice(b"next");
        assert_eq!(&plaintext[..], b"abc");
        assert_eq!(encrypted_tail.as_mut(), b"next");

        drop(plaintext);
        assert_eq!(
            returns.load(Ordering::Relaxed),
            0,
            "the mutable TLS remainder still owns the carrier"
        );
        drop(encrypted_tail);
        assert_eq!(returns.load(Ordering::Relaxed), 1);
    }
    ...
    #[test]
    fn published_windows_keep_only_a_disjoint_suffix_writable() {
        let returns = Arc::new(AtomicUsize::new(0));
        let mut carrier = WritableCarrierModel::new(8, Arc::clone(&returns));

        carrier.writable_tail()[..5].copy_from_slice(b"abcde");
        carrier.advance_initialized(5);
        let first = carrier.publish_prefix(3);
        carrier.writable_tail()[..3].copy_from_slice(b"fgh");
        carrier.advance_initialized(3);
        let second = carrier.publish_prefix(2);
        let third = carrier.publish_prefix(3);
    ...
    #[test]
    fn aligned_body_path_emits_one_full_carrier_or_final_tail() {
        let full_returns = Arc::new(AtomicUsize::new(0));
        let mut full = WritableCarrierModel::new(8, Arc::clone(&full_returns));
        full.writable_tail().copy_from_slice(b"abcdefgh");
        full.advance_initialized(8);
        let full = full.seal_full_or_body_end(false);
        assert_eq!(&full[..], b"abcdefgh");
        drop(full);
        assert_eq!(full_returns.load(Ordering::Relaxed), 1);

        let tail_returns = Arc::new(AtomicUsize::new(0));
        let mut tail = WritableCarrierModel::new(8, Arc::clone(&tail_returns));
        tail.writable_tail()[..3].copy_from_slice(b"xyz");
        tail.advance_initialized(3);
        let tail = tail.seal_full_or_body_end(true);
        assert_eq!(&tail[..], b"xyz");
        drop(tail);
        assert_eq!(tail_returns.load(Ordering::Relaxed), 1);
    }

    #[test]
    #[should_panic(expected = "a non-final aligned carrier must be full")]
    fn aligned_body_path_rejects_non_final_short_emission() {
        let returns = Arc::new(AtomicUsize::new(0));
        let mut carrier = WritableCarrierModel::new(8, returns);
        carrier.writable_tail()[..3].copy_from_slice(b"abc");
        carrier.advance_initialized(3);
        let _ = carrier.seal_full_or_body_end(false);
    }

    #[test]
    fn header_overread_is_copied_to_body_offset_zero_before_sealing() {
        let wire_read = b"HTTP/1.1 200 OK\r\nContent-Length: 8\r\n\r\nabc";
        let header_end = wire_read
            .windows(4)
            .position(|window| window == b"\r\n\r\n")
            .map(|start| start + 4)
            .unwrap();
        let overread = &wire_read[header_end..];

        let returns = Arc::new(AtomicUsize::new(0));
        let mut body = WritableCarrierModel::new(8, returns);
        body.writable_tail()[..overread.len()].copy_from_slice(overread);
        body.advance_initialized(overread.len());
        body.writable_tail().copy_from_slice(b"defgh");
        body.advance_initialized(5);

        let body = body.seal_full_or_body_end(false);
        assert_eq!(
            &body[..],
            b"abcdefgh",
            "header bytes are excluded and body begins at carrier offset zero"
        );
    }
