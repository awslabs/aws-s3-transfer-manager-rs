...
/// adaptive, transfer-aware latency deadline with its own retry loop
/// (`retry_guarded`/`classify_body_retry`). It also applies backpressure — a
/// chunk body may be intentionally left unpolled while the slot buffer or memory
/// budget is full. The SDK's body-level minimum-throughput guard cannot tell that
/// apart from a dead connection and would abort a healthy, backpressured chunk. A
/// client supplied directly via `client()` is left untouched (the caller owns its
/// configuration); this applies only to clients the transfer manager builds itself.
